...
import { ScanSearch, Clock, Terminal, AlertCircle, CheckCircle2, Loader2, XCircle, ChevronDown, ChevronRight } from "lucide-react";
...
const DETECTOR_STORY = {
  "Isolation Forest": "Looking for rows that are easiest to single out from the rest…",
  "Local Outlier Factor": "Checking each row against its neighbors for lonely spots…",
  "One-Class SVM": "Learning the shape of normal, then finding what crosses the line…",
};

export function AnomalyTrainStep({ detectors, status, progress, elapsed, logs, modelStates }) {
  ...
  const statusText = status === "completed" ? "Scan complete — unusual rows flagged" : (currentStory?.name ? DETECTOR_STORY[currentStory.name] : "Scanning your rows…");
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <div className="glass-panel glow-border relative flex items-center gap-6 rounded-2xl p-6 lg:col-span-2">
            ...
            <svg className="absolute inset-0 -rotate-90" viewBox="0 0 140 140">
              <defs>
                <linearGradient id="ringGradAmber" x1="0" y1="0" x2="1" y2="1">
                  <stop offset="0%" stopColor="oklch(0.76 0.19 75)" />
                  <stop offset="100%" stopColor="oklch(0.72 0.17 45)" />
                </linearGradient>
              </defs>
              <circle cx="70" cy="70" r="62" stroke="oklch(1 0 0 / 0.08)" strokeWidth="10" fill="none" />
              <circle
                cx="70" cy="70" r="62" stroke="url(#ringGradAmber)" strokeWidth="10" fill="none"
          ...
          <div className="flex-1">
            <div className="text-xs uppercase tracking-wider text-amber">{status === "completed" ? "Done — here are your unusual rows" : "Scanning for anomalies"}</div>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-4 text-sm">
              ...
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <span>{(modelStates || []).filter((m) => m.status === "completed").length} of {detectors?.length || 0} ways finished</span>
              </div>
            </div>
              ...
              <div className="animate-fade-in-up mt-4 flex items-start gap-2.5 rounded-xl border border-amber/30 bg-amber/10 p-3 text-xs">
                ...
                <div>
                  ...
                  <div className="mt-0.5 text-muted-foreground">
                    Large files get sampled to keep it fast; the scan is still chewing through the rows.
                  </div>
                </div>
              </div>
            )}
      ...
      <div className="glass-panel rounded-2xl p-5">
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <div className="text-xs text-muted-foreground">Each one scans the same rows its own way</div>
          </div>
          <span className="flex items-center gap-1.5 text-xs text-muted-foreground">
            <ScanSearch className="h-3.5 w-3.5 text-amber" /> {detectors?.length || 0} ways
          </span>
        </div>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {(modelStates || []).map((m, i) => (
            <div key={m.name} className={`relative overflow-hidden rounded-xl border p-3.5 interactive-card animate-fade-in-up stagger-${(i % 8) + 1} ${
              m.status === "running" ? "border-amber/50 bg-amber/5 animate-shimmer" :
              ...
              {DETECTOR_STORY[m.name] && (
                <div className="mb-2 text-[10.5px] leading-relaxed text-muted-foreground">
                  {statusLabel(m.status)} — {m.status === "completed" ? "finished its scan" : m.status === "failed" ? "hit a problem" : DETECTOR_STORY[m.name]}
                </div>
              )}
